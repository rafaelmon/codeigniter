rpycTareasDataStore = new Ext.data.Store({
    id: 'rpycTareasDataStore',
    proxy: new Ext.data.HttpProxy({
      ...
      reader: new Ext.data.JsonReader({
        ...
        {name: 'usuario_relacionado',   type: 'string', mapping: 'usuario_relacionado'},
...
paginador.bindStore(rpycTareasDataStore);

//botonesRpycTareasAction = new Ext.grid.ActionColumn({
//		width: 15,
//                editable:false,
//                menuDisabled:true,
//                header:'Acciónes',
...
rpycTareasColumnModel = new Ext.grid.ColumnModel(
    [{
        header: '#',
        readOnly: true,
        dataIndex: 'id_tarea',
...
//        renderer:showFecha,
        ...
        header: 'Usuario hallazgo',
        dataIndex: 'usuario_relacionado',
        sortable: true,
        width:  180,
        align:'left'
      },{
...
//      ,botonesRpycTareasAction
   ...
   rpycTareasListingGridPanel =  new Ext.grid.GridPanel({
        id: 'rpycTareasListingGridPanel',
        store: rpycTareasDataStore,
        cm: rpycTareasColumnModel,
        enableColLock:false,
...
//                handler: displayFormWindow,
    ...
    //            tooltip: 'e...',
            ...
            /*, '-', { 
                ...
                handler: confirmDeleteRpycTareas,   // Confirm before deleting
                iconCls:'remove',
...
//rpycTareasDataStore.load({params: {start: 0, limit: TAM_PAGINA}});

rpycTareasPanel = new Ext.Panel(
{
        collapsible: true,
        collapsed:true,
        split: false,
        ...
        layout: 'fit',
        items:[rpycTareasListingGridPanel]
});

//RpycTareasListingGridPanel.on('afteredit', guardartarea);
...
//            switch(result){
//                case 1:
//                    rpycTareasDataStore.commitChanges();
//                    rpycTareasDataStore.reload();
//                    break;
//                default:
...
//}
	...
	rpycTareasListingGridPanel.setHeight(altura);
	
	Ext.getCmp('browser').on('resize',function(comp){
		rpycTareasListingGridPanel.setWidth(this.getSize().width);
		rpycTareasListingGridPanel.setHeight(Ext.getBody().getSize().height - 60);
...
// 
	...
	var encoded_array_v = Ext.encode(values);
//	var encoded_array_v = Ext.encode(values);
    rpycTareasDataStore.setBaseParam('filtros',encoded_array_v);
    rpycTareasDataStore.load();
}
...
//    store1.load();
    rpycTareasDataStore.setBaseParam('filtros','');
    rpycTareasDataStore.load();
}
rpycTareasListingGridPanel.on('celldblclick', abrir_popup_tareas_rpyc);
function abrir_popup_tareas_rpyc(grid ,  rowIndex, columnIndex,  e){
...
//        data.revisores="No LLeva"
    ...
    winDoc = new Ext.Window({
            title: 'Detalle de Tarea asociada a la RPyC ',
            closable: true,
            modal:true,
            //closeAction: 'hide',
            width: 800,
