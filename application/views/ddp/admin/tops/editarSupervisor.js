...
function editarDdpSupervisor(){Ext.Ajax.request({url: LINK_GENERICO+'/sesion',method: 'POST',success: function(response, opts){var result=parseInt(response.responseText);switch (result){case 0:case '0':location.assign(URL_BASE_SITIO+"admin");break;case 1:case '1':
go_editarDdpSupervisor();break;}},failure: function(response) {location.assign(URL_BASE_SITIO+"admin");}});}


function go_editarDdpSupervisor()
{
    
    if(isDdpSupervisorTopFormValid()){
       //msgProcess('Creando TOP...');
      Ext.Ajax.request({   
        waitMsg: 'Por favor espere...',
        url: CARPETA+'/editar_supervisor',
        params: {
            id_top          : id_top,         
...
function isDdpSupervisorTopFormValid()
{	  
...
function dFW_ddpEditaSupervisor(grid,rowIndex,colIndex,item,event)
{    
...
//    var empleado=grid.getStore().getAt(rowIndex).json.empleado;
 ...
 ddpTopCreateForm = new Ext.FormPanel({
        ...
        items: [{
            ...
            layout:'form',
            border:false,
            items:[supervisorCombo]
        }],
        buttons: [{
            text: 'Guardar',
            handler: editarDdpSupervisor
            },{
            text: 'Cancelar',
            handler: function(){
                ddpTopCreateWindow.close();
   ...
   ddpTopCreateWindow= new Ext.Window({
        ...
        title: 'Editar Supervisor de: '+usuario,
        closable:false,
        modal:true,
        width: 330,
        height: 150,
