<?php
class Consecuencias extends CI_Controller
{
    private $modulo=45;
    private $user;
    private $permisos;
...
//            echo "<pre>Origen".print_r($this,true)."</pre>";
	...
	public function index()
	{
            ...
            $this->load->view('cpp/consecuencias/listado',$variables);
	}
	...
	public function listado()
	{
            if ($this->permisos['Listar'])
            {
                ...
                $listado = $this->consecuencias->listado($start, $limit, $sort,$dir,$busqueda,$campos);
                echo $listado;
            }
            else
        ...
        public function insert()
	{
            if ($this->permisos['Alta'])
            {
                ...
                $datos['consecuencia'] = $this->input->post("consecuencia");
                $datos['descripcion'] = $this->input->post("descripcion");
//                echo "<pre>".print_r($datos,true)."</pre>";die();
                if ($datos['consecuencia'] != '')
                {
                    $this->load->model('cpp/cpp_consecuencias_model','consecuencias',true);	
                    if ($this->consecuencias->insert($datos))
                        echo 1;
                    else
                        echo 0;
                }
        ...
        public function update()
	{
            ...
            {
                $id = $this->input->post("id_consecuencia");
                ...
                if (count($campos)>0)
                {
                    ...
                    if ($this->consecuencias->update($id,$datos))
                    {
                        echo 1;
                    }
                    else
