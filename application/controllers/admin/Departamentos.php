...
class Departamentos extends CI_Controller
{
	...
	public function index()
	{
            ...
            $this->load->view('admin/departamentos/listado',$variables);
	}
    ...
    //                            echo "<pre>".print_r($campos,true)."</pre>";
                    ...
                    $listado = $this->departamentos->listado($start, $limit, $filtro, $campos, $sort, $dir);
                    echo $listado;
                }
                else
	...
	public function modificar()
	{
            if ($this->permisos['Modificacion'])
            {
                ...
                $this->load->model('departamentos_model','departamentos',true);
                if($campo[0]=="departamento")
                {
                    $control["departamento"]=$valor[0];
                    $control["id_gerencia"]=$_POST["id_gerencia"];
                    $datos[$campo[0]]=$valor[0];
                        if(!$this->departamentos->check_departamentoGerencia($control))
                        {
                            if(!$this->departamentos->edit($_POST["id"],$datos))
                            echo 0;
                        else
                            echo 1;
	...
	public function insert()
	{
            if ($this->permisos['Alta'])		
            {
                ...
                $datos['id_gerencia']   =$_POST["id_gerencia"];
                $this->load->model('departamentos_model','departamentos',true);
                //antes de insertar verifico que no exita el registro
                if(!$this->departamentos->check_departamentoGerencia($datos))
                {
                    if($this->departamentos->insert($datos))
                        echo 1; //todo OK
                    else
        ...
        public function gerencias_combo()
	{
        ...
        public function dameDepartamente()
	{
            $id_empresa= $this->input->post("id_empresa");
            $this->load->model('departamentos_model','departamentos',true);
            $var=$this->departamentos->dameComboGerencias($id_empresa);
